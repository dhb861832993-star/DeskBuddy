...
- **自有 USN 引擎（v2.3 新增）**：默认走 NTFS USN 日志独立建索引（Listary 同技术路线），**完全不依赖 Windows Search**——即使系统索引损坏/禁用也能秒搜千万级文件；首次构建约数十秒，之后毫秒级搜索
- **搜索后端可选**（设置 → 搜索 → 搜索后端）：`auto`（优先 USN 引擎 → Windows Search → 内置索引）/ `usn`（自有引擎）/ `wsearch`（Windows Search 系统索引）/ `builtin`（内置索引，上限 30 万条）
- **自动加索引范围（v2.3 新增）**：大目录（>3000 个文件）首次加入搜索范围时自动注册到 Windows Search 索引（COM API），并在右下角显示索引进度条（仅 `wsearch` 后端显示）
