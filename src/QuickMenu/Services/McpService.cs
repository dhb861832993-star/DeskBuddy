...
/// <summary>
/// QuickMenu 的 MCP 服务（stdio）：以 <c>QuickMenu.exe --mcp</c> 启动，
...
public static class McpService
{
    ...
    public static async Task RunAsync(CancellationToken ct)
    {
        ...
        if (!cfg.McpEnabled)
        {
            Console.Error.WriteLine("QuickMenu MCP 未启用：请在 QuickMenu 设置中开启「AI 快捷添加（MCP）」");
            return;
        }
        ...
        {
            ServerInfo = new Implementation { Name = "QuickMenu", Version = "1.8.9" },
            ServerInstructions = "QuickMenu 快捷菜单 MCP 服务：AI 工具可以查看（list_menu_items）、添加（add_menu_item）、删除（remove_menu_item）本机 QuickMenu 的快捷菜单条目。添加/删除立即生效，无需重启。",
        };
        options.ToolCollection = new McpServerPrimitiveCollection<McpServerTool>
        {
            ...
            McpServerTool.Create(RemoveMenuItem, new McpServerToolCreateOptions
            {
                ...
                Description = "按名称删除一个 QuickMenu 快捷菜单条目并立即生效。删除前建议先 list_menu_items 确认名称。",
            }),
            McpServerTool.Create(ListMenuItems, new McpServerToolCreateOptions
            {
                Name = "list_menu_items",
                Description = "列出 QuickMenu 快捷菜单的所有条目（name/type/path/args/keywords/icon）。",
                ReadOnly = true,
            }),
        };
    ...
    private static string McpPipeDown() =>
        "{\"ok\":false,\"message\":\"无法连接 QuickMenu 主程序（请确认 QuickMenu 已启动，且设置中已开启 AI 快捷添加）\"}";
