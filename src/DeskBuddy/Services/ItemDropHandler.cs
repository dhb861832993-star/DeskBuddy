...
public static class ItemDropHandler
{
    ...
    public static List<DeskBuddyItem> FromDrop(System.Windows.IDataObject data)
    {
    ...
    public static List<DeskBuddyItem> FromFiles(IEnumerable<string> paths)
    {
        var result = new List<DeskBuddyItem>();
    ...
    private static DeskBuddyItem? FromFile(string path)
    {
        try
        {
            ...
            if (ext == ".lnk")
            {
                ...
                if (!string.IsNullOrEmpty(target))
                {
                    if (Directory.Exists(target))
                        return new DeskBuddyItem { Name = name, Type = "folder", Path = target };
                    if (File.Exists(target))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(args))
                            return new DeskBuddyItem { Name = name, Type = "file", Path = path };
                        return new DeskBuddyItem { Name = name, Type = "app", Path = target };
                    }
                }
                return new DeskBuddyItem { Name = name, Type = "file", Path = path };
            }
            ...
            if (ext == ".url")
            {
                ...
                {
                    return new DeskBuddyItem { Name = name, Type = "url", Path = url };
                }
                return new DeskBuddyItem { Name = name, Type = "file", Path = path };
            }
            ...
            if (File.Exists(path))
            {
                ...
                {
                    return new DeskBuddyItem { Name = Path.GetFileNameWithoutExtension(path), Type = "app", Path = path };
                }
                return new DeskBuddyItem { Name = name, Type = "file", Path = path };
            }
        }
